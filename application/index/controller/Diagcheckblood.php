...
class Diagcheckblood extends Rest
{
    ...
    public function edit() {
        ...
        $retData = apiDiagCheck::apiDiagCheckGet(Session::get('attest'),2,$diag_check_type);
        if($retData) {
            if($retData['ret_code'] == 0) {
...
//        $check_type = Request::instance()->param('check_type');
        ...
        $retData = apiDiagCheck::apiDiagCheckAdd($attest,2,$cn_name,$en_name,$expression,$remark);
        if( $retData ) {
            if( $retData['ret_code'] == 0 ) {
                print 0;
            }
...
//        $check_type = Request::instance()->param('check_type');
        $diag_check_type = Request::instance()->param('diag_check_type');
        $retData = apiDiagCheck::apiDiagCheckDrop($attest,2,$diag_check_type);
        if( $retData ) {
            if( $retData['ret_code'] == 0 ) {
                print 0;
            }
    ...
    public function ajax_count()
    {
        ...
        $retData = apiDiagCheck::apiDiagCheckList($attest, 2, 0, 0);
        if( $retData ) {
            if( $retData['ret_code'] == 0 ) {
...
//        $get_num = Request::instance()->param('get_num');
        ...
        $start = 0;
        for($i=0;$i<1000;$i++){
            $retData = apiDiagCheck::apiDiagCheckList($attest,2,$start,10);
            if ($retData) {
                if ($retData['ret_code'] == 0) {
...
//        $check_type = Request::instance()->param('check_type');
        ...
        $expression = Request::instance()->param('expression');
        $remark = Request::instance()->param('remark');
        $retData = apiDiagCheck::apiDiagCheckSet($attest,2,$diag_check_type,$info_ver,$cn_name,$en_name,$expression,$remark);
        if( $retData ) {
            if( $retData['ret_code'] == 0 ) {
                print 0;
            }
