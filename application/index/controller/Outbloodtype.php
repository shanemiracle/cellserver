...
class Outbloodtype extends Rest
{
    ...
    public function add()
    {
        ...
        $retData = apiCellType::apiCellTypeGet($attest, 2, $father_cell_type);
        $c_r = 0;
        $c_g = 0;
        $c_b = 0;
        ...
        return (new View())->fetch('/outbloodtype/add', ['father_cell_type' => $father_cell_type, 'father_cell_name' => $father_cell_name,
    ...
    public function index()
    {
        ...
        $retData = apiCellType::apiCellTypeFacherList(Session::get('attest'), 2, $cur_cell_type);
        if ($retData) {
            if ($retData['ret_code'] == 0) {
                $data = $retData['data'];
                $count = count($data);
        ...
        return (new View())->fetch('/outbloodtype/index', ['cur_cell_type' => $cur_cell_type, 'cur_cell_name' => $cur_cell_name,
    ...
    public function ajax_add()
    {
        ...
        $retData = apiCellType::apiCellTypeAdd($attest, 2, $father_cell_type, $cn_name, $en_name, $abb_name, $size_max * 10, $size_min * 10, $remark, $file_id_big,$file_id_small,
    ...
    public function ajax_drop()
    {
        $attest = Session::get('attest');
        $cell_type = Request::instance()->param('cell_type');
        $retData = apiCellType::apiCellTypeDrop($attest, 2, $cell_type);
        if ($retData) {
            if ($retData['ret_code'] == 0) {
                print 0;
            } else {
    ...
    public function ajax_count()
    {
        ...
        $retData = apiCellType::apiCellTypeList($attest, 2, 0, 0);
        if( $retData ) {
            if( $retData['ret_code'] == 0 ) {
    ...
    public function ajax_son_list()
    {
        ...
        $retData = apiCellType::apiCellTypeSonList($attest, 2, $cur_cell_type);
        if ($retData) {
            if ($retData['ret_code'] == 0) {
                $data = $retData['data'];
    ...
    public function ajax_edit()
    {
        ...
        $retData = apiCellType::apiCellTypeSet($attest, 2, $cell_type, $info_ver, $cn_name, $en_name, $abb_name, $size_max * 10, $size_min * 10, $remark,$file_id_big,$file_id_small,
            $color_r,$color_g,$color_b,$is_special);
        if ($retData) {
            if ($retData['ret_code'] == 0) {
                print 0;
    ...
    public function ajax_ten_add()
    {
        ...
        $retData = apiCellType::apiTenCellTypeAdd($attest, 2,$data);
        if ($retData) {
            if ($retData['ret_code'] == 0) {
                $data = $retData['data'];
            }
    ...
    {
        $attest = Session::get('attest');
        $retData = apiCellType::apiTenCellTypeDrop($attest, 2);
        if ($retData) {
            if ($retData['ret_code'] == 0) {
                print 0;
            } else {
    ...
    public function ajax_ten_get()
    {
        $attest = Session::get('attest');
        $data = [];
        $retData = apiCellType::apiTenCellTypeGet($attest, 2);
        if ($retData) {
            if ($retData['ret_code'] == 0) {
                $data = $retData['data'];
            }
    ...
    public function ajax_ten_set()
    {
        ...
        $retData = apiCellType::apiTenCellTypeSet($attest, 2,$info_ver, $data);
        if ($retData) {
            if ($retData['ret_code'] == 0) {
                $data = $retData['data'];
            }
